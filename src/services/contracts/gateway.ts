...
export type ApiDefinitionMode="openapi"|"manual"|"builtin";
export interface ApiDefinitionSummary { mode:ApiDefinitionMode; connector?:"evernote"; noteStoreUrl?:string }

export interface Capability { id:string; name:string; description:string; risk:"read"|"write"|"delete"; permission:Permission }
export interface Service { id:string; name:string; code:string; description:string; category:string; type:"MCP"|"API"|"邮箱"; status:Status; transport:string; updatedAt:string; capabilities:Capability[]; apiDefinition?:ApiDefinitionSummary }
